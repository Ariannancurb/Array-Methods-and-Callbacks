...
function getFinals(data) {
   const finalsTeam = data.filter(function(item){
...
function getWinnersByYear(array, getFinalsCB, getYearsCB, getWinnersCB) {
    ...
    return winners.map((item, index)=> `In ${years[index]}, ${country[index]} won the world cup! `
});
...
function getAverageGoals(data) {
   const averageHomeGoal = data.reduce(function(acc,item){
       return acc + item['Home Team Goals'] + item ['Away Team Goals'];
   }, 0)
   return(averageHomeGoal/ data.length).toFixed(2);
}
